    ...
    if isinstance(ckpt, dict):
        for k in ["state_dict", "model_state_dict", "model"]:
            if k in ckpt and isinstance(ckpt[k], dict):
                return ckpt[k]
        # sometimes it's already a state_dict dict
        if all(isinstance(v, torch.Tensor) for v in ckpt.values()):
            return ckpt
    raise ValueError(f"Unsupported checkpoint format in {path}")

def normalize_state_keys(state: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    # Handles common training-time prefixes / naming differences:
    # - DataParallel: "module."
    # - Our old training code: "base_model." instead of "backbone."
    new_state: Dict[str, torch.Tensor] = {}
    for k, v in state.items():
        if k.startswith("module."):
            k = k[len("module."):]
        if k.startswith("base_model."):
            k = "backbone." + k[len("base_model."):]
        new_state[k] = v
    return new_state
...
def infer_output_dim_from_state(state: Dict[str, torch.Tensor]) -> int:
    ...
    raise KeyError("Could not infer output dim from checkpoint (no regressor.*.weight found).")
...
def build_model(out_dim: int) -> nn.Module:
    ...
    def forward(x):
        ...
        return model.regressor(feats)

    model.forward = forward  # type: ignore
    return model
...
def main() -> None:
    ...
    ap.add_argument("--num_params", type=int, default=0, help="Override output dim. If 0, infer from checkpoint.")
    ap.add_argument("--target_cols", type=str, default="", help="Comma-separated target cols override.")
    args = ap.parse_args()
    ...
    if args.target_cols.strip():
        ...
        if out_dim not in DEFAULT_COLS_BY_D:
            raise ValueError(
                f"Unsupported out_dim={out_dim}. Provide --target_cols explicitly."
            )
    ...
    # report file
    gt_cols = ["filename"] + target_cols
    gt_df = df[gt_cols].copy()
    ...
    for i, c in enumerate(target_cols):
        ...
        abs_err_cols.append(f"abs_err_{c}")
        sq_err_cols.append(f"sq_err_{c}")
    ...
    for c in target_cols:
        ...
        plt.xlabel(f"true {c}")
        plt.ylabel(f"pred {c}")
        plt.title(f"ResNet18 ({out_dim}D) {args.split}: true vs pred ({c})")
        plt.tight_layout()
        plt.savefig(outdir / f"resnet18_{out_dim}d_{args.split}_scatter_{c}.png", dpi=200)
        plt.close()
        ...
        plt.ylabel("count")
        plt.title(f"ResNet18 ({out_dim}D) {args.split}: abs error histogram ({c})")
        plt.tight_layout()
        plt.savefig(outdir / f"resnet18_{out_dim}d_{args.split}_hist_{c}.png", dpi=200)
        plt.close()
